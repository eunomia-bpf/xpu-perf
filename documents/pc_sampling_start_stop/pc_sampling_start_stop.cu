    ...
    char *argv[])
{
    ...
    // Create context only if one doesn't exist
    if (cuCtx == NULL)
    {
        CUdevice cuDevice;
        DRIVER_API_CALL(cuDeviceGet(&cuDevice, deviceNum));

        // Use cuDevicePrimaryCtxRetain instead of cuCtxCreate for compatibility
        DRIVER_API_CALL(cuDevicePrimaryCtxRetain(&cuCtx, cuDevice));
    }

    DoPCSampling(cuCtx);

    // Only destroy if we created it
    // Note: For primary context, we should use cuDevicePrimaryCtxRelease
    // but for simplicity we'll let cudaDeviceReset handle cleanup
