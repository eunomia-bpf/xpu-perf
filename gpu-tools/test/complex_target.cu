...
#define MEMORY_ALLOCATION_CALL(var)                                           \
    do {                                                                       \
        if (var == nullptr) {                                                 \
            fprintf(stderr, "%s:%d: error: memory allocation failed for %s.\n", \
    ...
    char *argv[])
{
    ...
    for (int device = 0; device < numDevices; device++)
    {
        ...
        // For CUDA 13.0+, we need to use the v4 API with CUctxCreateParams
        #if CUDA_VERSION_NUM >= 130000
            CUctxCreateParams params = {};
            DRIVER_API_CALL(cuCtxCreate(&(deviceData[device].context), &params, 0, deviceIds[device]));
        #else
