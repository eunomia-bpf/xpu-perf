import { FlameStackEntry, FlameData, FlameTreeNode, FileInfo, ThreadStats } from '@/types/flame.types';

export class FlameGraphDataLoader {
  private data: FlameData = {};

  /**
   * Parse folded file content and extract flame graph data
   */
  parseFoldedContent(content: string, threadName: string): FlameStackEntry[] {
    const lines = content.split('\n');
    const flameData: FlameStackEntry[] = [];
   ...
   */
  async loadFiles(fileInfos: FileInfo[]): Promise<FlameData> {
  ...
  /**
   * Build hierarchical flame tree from flat data
   */
  buildFlameTree(data: FlameStackEntry[]): Record<string, FlameTreeNode> {
    const tree: Record<string, FlameTreeNode> = {};
   ...
   */
  getVisualizationData(): FlameData {
    return this.data;
  }
   ...
   */
  filterByMinCount(data: FlameData, minCount = 10): FlameData {
    const filtered: FlameData = {};
   ...
   */
  normalizeFunctionNames(data: FlameData): FlameData {
    const normalized: FlameData = {};
  ...
  /**
   * Get comprehensive statistics for all threads
   */
